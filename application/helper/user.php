...
{	
	...
	function saveUserSession($user)
	{
		...
		if (!empty($user))
		{
            ...
            $use['role'] = 'user';
            //生成一个id
            $key=md5($user['id']. $_SERVER['REQUEST_TIME']);

            //将session存到cookie
            $this->memcache->__set($key,$use);
            //保存cookie
            $this->cookie->__set('TWG',$key);
	...
	function createUserData()
	{
		...
		while(!empty($this->model('user')->where('invit=?',[$invit])->find()));
		...
		if (!empty($share_uid))
		{
			$o_user = $this->model('user')->where('id=?',[$share_uid])->find();
		...
		if (!empty($user_source))
		{
			...
			$source_uid = $this->model('source')->where('id=?',[$source])->find();
			if(empty($source_uid))
			{
				$source = NULL;
			}else{
				if($source_uid['type']==0){
					...
					$oid = $source_uid['uid'];
					$suser = $this->model('user')->where('id=?',[$source_uid['uid']])->find([
							'master',
							'o_master'
					]);
					if($suser['master']==1){
	...
	/**
	 * 验证用户的手机号和密码是否正确
	 * @param unknown $password  明文密码
	 * @param unknown $encrypted_password 密文密码
	 * @param unknown $salt 盐值
	 * @return boolean
	 */
