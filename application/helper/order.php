...
class order extends base
{
	...
	public function departByStore($orderno)
	{
		...
		// 判断是否已经拆分过了
		if (! empty($this->model('suborder_store')
			->where('main_orderno=?', [
			$orderno
		])
			->find()))
		...
		// 计算订单下的仓库数量
		$store = [];
		$order_package = $this->model('order_package')
			->where('orderno=?', [
			$orderno
		])
			->select();
	...
	private function totalamount()
	{
		...
		foreach ($this->_product as $p)
		{
			...
			$store = $this->model('store')->where('id=? and isdelete=?',[$product['store'],0])->find();
			if (!empty($store['erp']))
			{
				$this->_need_erp = 1;
			}
		...
		if (! empty($this->_coupon))
		{
			$coupon = $this->model('coupon')
				->where('(endtime>=? or endtime=0) and used=? and isdelete=? and id=? and uid=?', [
...
;
	...
	function createPackageData()
	{
		...
		foreach ($this->_product as $p)
		{
			...
			if (! empty($p['content']))
			{
				$price_collection = $this->model('collection')
	...
	function getProduct($orderno)
	{
		...
		$product = $this->model('order')
			...
			->select('order_product.pid,order_product.num,order_product.content');
		foreach ($product as $p)
		{
			if (! empty($p['content']))
			{
				...
				if (! empty($collection))
				{
					$temp_product = $this->model('product')
						->where('id=?', [
						$p['pid']
					])
						->find();
	...
	function payedOrder($orderno, $pay_type, $pay_number, $pay_money, array $options = [])
	{
		...
		if (! empty($order) && $order['pay_status'] == '0' && $order['orderamount'] == $pay_money)
		{
			...
			if (class_exists($class_name, true) && method_exists($class_name, 'payedOrder') && is_callable([
				$class_name,
				'payedOrder'
			]))
			{
	...
	function refund($orderno, $order_product_id = NULL)
	{
		...
		if (! empty($order))
		{
			...
			$this->model('refund')
				...
				->limit(1)
				->update('reason', json_encode($response, JSON_UNESCAPED_UNICODE));
