...
class order extends common
{
	...
	function detail()
	{
		...
		$order = $this->model('order')->where('orderno=?', [
			$orderno
		])->find([
			'order.orderno',
			'order.address_province as province',
			'order.address_city as city',
			'order.address_county as county',
			'order.address_address as address',
			'order.address_name as name',
			'order.address_telephone as telephone',
			'order.address_identify as identify',
			'order.goodsamount',
			'order.feeamount',
			'order.taxamount',
			'order.discount',
		...
		if (! empty($order))
		{
			...
			if ($order['pay_status'] == 1)
			{
				...
				if ($order['way_status'] == 1)
				{
					...
					{
						$iorder = $this->model("order_package")
							->where("orderno=?", [
							$orderno
						])
							->find();
	...
	function mylists()
	{
		...
		$pay_status = $this->model('order')
			->where("uid=? and pay_status=0 and status=1", [
			$uid
		])
			->count();
		...
		$way_status = $this->model('order')
			->where("uid=? and (pay_status=1 or pay_status=4) and way_status=0 and status=1", [
			$uid
		])
			->count();
		...
		$receive = $this->model('order')
			->where("uid=? and  (pay_status=1 or pay_status=4) and way_status=1 and  receive=0 and status=1", [
			$uid
		])
			->count();
		// 获取公告
