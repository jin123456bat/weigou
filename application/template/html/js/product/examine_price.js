...
var price = datatables({
	...
	columnDefs:[{
		...
		targets:7,
		render:function(data,full){
			if(full.examine_price==0)
			{
				return '<font style="color:#FF6600;">待审核</font>';
			}
			else if(full.examine_price==2)
			{
...
$('#price table').on('click','.editBtn',function(){
	var id = $(this).data('id');
	$.post('./index.php?m=ajax&c=product&a=examine2_price',{id:id},function(response){
		if(response.code==1)
		{
			window.location = './index.php?c=html&a=product_edit&type=examine_price&id='+id;
		}
	});
	return false;
	...
	$.post('./index.php?m=ajax&c=product&a=examine_price_pass',{id:id},function(response){
		if(response.code==1)
		{
			tr.remove();
		}
	...
	$('#examineModal input[name=type]').val('examine_price');
	$('#examineModal').modal('show');
}).on('click','.lookBtn',function(){
	window.open('/index.php?c=index&a=product&id='+$(this).data('id'));
});

$('#price .search').on('submit',function(){
	price.addAjaxParameter('status',$(this).find('select').val());
	price.search($(this).find('input').val());
	return false;
});