...
defined('ROOT') or define('ROOT', "/var/www/html");
include '../../global.php';
...
else
{
	...
	{
		$response->setCode(302);
		$url = 'http://'.$_SERVER['HTTP_HOST'].'/index.php?c=order&a=payment&orderno='.$state.'&openid='.$content->openid.'&trade_type='.$trade_type;
		$response->addHeader('Location',$url);
		$response->send();
	}
	else
