...
class admin extends view
{
    ...
    function product_up()
    {
        ...
        if ($_FILES["file"]) {
            ...
            if ($this->post("ls", '', 'trim') == "price") {//判断是否是price
                ...
                foreach ($data as $d) {
                    ...
                    if (!$product) {
                        ...
                        if ($d[2] == 0 || empty($d[2]) || $d[3] == 0 || empty($d[3]) || $d[4] == 0 || empty($d[4])) {
                            ...
                            $isbl = $this->model("product")->where("id=?", [$product['id']])->limit(1)->update([
                                'oldprice' => $d[5],
                                'v1price' => $d[3],
                                'v2price' => $d[4],
                                'price' => $d[2],
                            ...
                            if ($isbl) {
                                //excle也保存一句话
                                $content = "修改成功\tsku:" . $product['sku']
                                    . "\t 条形码：" . $d[0]
                                    . "\t" . $product['name']
                                    . "\t原价：" . $product['oldprice'] . "变更为：" . $d[5]
                                    . ",V0：" . $product['price'] . "变更为：" . $d[2]
                                    . ",V1：" . $product['v1price'] . "变更为：" . $d[3]
                                    . ",V2：" . $product['v2price'] . "变更为：" . $d[4]
                                    . ",起售数量：" . $product['selled'] . "变更为：" . $d[6]
                                    . ",进价：" . $product['inprice'] . "变更为：" . $d[1]
                                    . "\n";
                                echo $content;
