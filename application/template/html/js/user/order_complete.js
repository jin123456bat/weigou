var complete = datatables({
	table:$('#order_complete table'),
	ajax:{
...
//批量操作
$('#order_complete .multipleBtn').on('click',function(){
	...
	if($(this).parents('td').find('.multipleSelect').val() == 'delete')
	{
		bootbox.confirm({
			...
			callback: function(result,a) {
				if(result) {
					complete.addAjaxParameter('customActionType','group_action');
					complete.addAjaxParameter('id',id);
					complete.addAjaxParameter('customActionName','remove');
					complete.reload();
				}
			},
		});
