...
class mobile extends view
{
	...
	function orderinfo()
	{
		...
		if (! empty($orderno))
		{
			...
			if (! empty($order))
			{
				 ...
				 */
				// 获取物流信息
				$courier = $this->model("order_package")
					->where("orderno=?", [
					$orderno
				])
					->find();
	...
	function myinvit()
	{
		...
		else
		{
			...
			$this->assign('weibo_appkey', $this->model('system')
				->get('appkey', 'weibo'));
			return $this;
		}
	}
	...
	function teaminfo()
	{
		...
		if (! empty($id))
		{
			...
			if (! empty($user))
			{
				...
				$dayteam = $this->model('user')
					->where('oid=? and invittime>?', [
					$id,
					$time
				])
					->find('count(*)');
	...
	function theme()
	{
		...
		if (! empty($id))
		{
			$theme = $this->model('theme')
				->table('upload', 'left join', 'theme.logo=upload.id')
				->where('theme.id=?', [
				$id
			])
				->find('theme.id,upload.path as logo,theme.title');
			...
			if (! empty($theme))
			{
				...
				foreach ($subtheme as &$st)
				{
					$product = $this->model('subtheme_product')
						...
						->where('subtheme_product.subtheme_id=?', [
						$st['id']
					])
						->select([
						'product.id',
		...
		if (! empty($task_user))
		{
			...
			if (! empty($uid))
			{
				if (empty($o_orderno))
				{
					$order = $this->model('task_user')
						...
						->where('task_user.tid=? and order.uid=? and task_user.o_orderno is null', [
						$order['tid'],
						$uid
					])
						->find();
				...
				else
				{
					$order = $this->model('task_user')
						->table('`order`', 'left join', 'order.orderno=task_user.orderno')
						->where('task_user.tid=? and order.uid=? and task_user.o_orderno=?', [
						$order['tid'],
						$uid,
						$o_orderno
					])
			...
			if (empty($task_user['o_orderno']))
			{
				...
				$complete_user_gravatar = $this->model('task_user')
					...
					->where('task_user.o_orderno=? and order.pay_status=? and order.status=?', [
					$main_order['orderno'],
					1,
					1
				])
			...
			else
			{
				...
				$complete_user_gravatar = $this->model('task_user')
					...
					->table('user', 'left join', 'user.id=order.uid')
					->table('upload', 'left join', 'upload.id=user.gravatar')
					->where('task_user.o_orderno=? and order.pay_status=?', [
					$task_user['o_orderno'],
					1
				])
	...
	function task()
	{
		...
		else
		{
			$product = $this->model('task')
				...
				->where('(auto_stock=? and stock>?) or auto_stock=?', [
				1,
				0,
				0
			])
				...
				->where('(product.auto_status=? and product.status=?) or (product.auto_status=? and product.avaliabletime_from < ? and product.avaliabletime_to > ?)', [
				0,
				1,
				1,
			...
			$order = $this->model('task_user')
				...
				->table('product', 'left join', 'product.id=task.pid')
				->table('store', 'left join', 'product.store=store.id')
				->where('order.uid=? and order.status=?', [
				$uid,
				1
			])
				->select([
			...
			foreach ($order as &$product)
			{
				...
				if (empty($product['o_orderno']))
				{
					$complete_user_num = $this->model('task_user')
						->table('`order`', 'left join', 'order.orderno=task_user.orderno')
						->where('task_user.o_orderno=? and order.status=? and order.pay_status=?', [
						$product['orderno'],
						1,
						1
					])
	...
	function cart()
	{
		...
		if (! empty($uid))
		{
			...
			$store = $this->model('cart')
				...
				->table('store', 'left join', 'product.store=store.id')
				->where('product.status=1 and cart.uid=?', [
				$uid
			])
				->groupby('store.id')
			...
			if ($store)
			{
				...
				$cartl = $this->model("cart")
					->table('product', 'left join', 'product.id=cart.pid')
					->where('cart.uid=? and ((product.status=0 and product.auto_status=0) or (auto_status=1 and (product.avaliabletime_from>? or product.avaliabletime_to<?)))', [
					$uid,
					$_SERVER['REQUEST_TIME'],
					$_SERVER['REQUEST_TIME']
				])
	...
	function edit_address()
	{
		...
		else
		{
			...
			$address = $this->model('address')
				->where('address.id=? and address.uid=? and address.isdelete=?', [
				$id,
				$uid,
				0
			])
	...
	function product()
	{
		...
		if (! empty($product))
		{
			...
			// 商品上下架
			if (($product['auto_status'] == 1 && ($_SERVER['REQUEST_TIME'] < $product['avaliabletime_from'] || $_SERVER['REQUEST_TIME'] > $product['avaliabletime_to'])) || ($product['auto_status'] == 0 && $product['status'] == 0))
			{
	...
	function index()
	{
		$carousel = $this->model('carousel')
			...
			->where('carousel.isdelete=? and carousel.position=?', [
			0,
			'index'
		])
			->orderby('sort', 'asc')
		...
		$theme = $this->model('subtheme_product')
			->where("subtheme_product.subtheme_id=27")
			->select([
			'product_id'
		]);
