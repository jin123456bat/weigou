...
$('#completed_search').on('submit',function(){
	completed.addAjaxParameter('status',$(this).find('select').val());
	completed.search($(this).find('input').val());
	return false;
});

//批量操作
$('#completed .multipleBtn').on('click',function(){
	...
	if($(this).parents('td').find('.multipleSelect').val() == 'delete')
	{
		bootbox.confirm({
			...
			callback: function(result,a) {
				if(result) {
					completed.addAjaxParameter('customActionType','group_action');
					completed.addAjaxParameter('id',id);
					completed.addAjaxParameter('customActionName','remove');
					completed.reload();
				}
			},
		});
