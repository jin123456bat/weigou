var waying = datatables({
	table:$('#order_waying table'),
	ajax:{
...
//批量操作
$('#order_waying .multipleBtn').on('click',function(){
	...
	if($(this).parents('td').find('.multipleSelect').val() == 'delete')
	{
		bootbox.confirm({
			...
			callback: function(result,a) {
				if(result) {
					waying.addAjaxParameter('customActionType','group_action');
					waying.addAjaxParameter('id',id);
					waying.addAjaxParameter('customActionName','remove');
					waying.reload();
				}
			},
		});
