var sending = datatables({
	...
	//订单状态
	{
		...
		data:'erp_sending_num',
		name:'(select count(*) from suborder_store where suborder_store.main_orderno=order.orderno and erp=1)',
		visible:false,
	},{
		data:'erp_not_sending_num',
		name:'(select count(*) from suborder_store where suborder_store.main_orderno=order.orderno and erp=0)',
		visible:false,
	},{
		data:'need_erp',
		name:'order.need_erp',
