...
var examine_stock = datatables({
	...
	columnDefs:[{
		...
		targets:7,
		render:function(data,full){
			if(full.examine_stock==0)
			{
				return '<font style="color:#FF6600;">待审核</font>';
			}
			else if(full.examine_stock==2)
			{
...
$('#stock table').on('click','.editBtn',function(){
	var id = $(this).data('id');
	$.post('./index.php?m=ajax&c=product&a=examine2_stock',{id:id},function(response){
		if(response.code==1)
		{
			window.location = './index.php?c=html&a=product_edit&type=examine_stock&id='+id;
		}
	});
	return false;
	...
	$.post('./index.php?m=ajax&c=product&a=examine_stock_pass',{id:id},function(response){
		if(response.code==1)
		{
			tr.remove();
		}
	...
	$('#examineModal input[name=type]').val('examine_stock');
	$('#examineModal').modal('show');
}).on('click','.lookBtn',function(){
	window.open('/index.php?c=index&a=product&id='+$(this).data('id'));
});

$('#stock .search').on('submit',function(){
	examine_stock.addAjaxParameter('status',$(this).find('select').val());
	examine_stock.search($(this).find('input').val());
	return false;
});