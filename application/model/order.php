...
class orderModel extends model
{
	...
	function datatables($post)
	{
		...
		if (isset($post['action']) && $post['action'] === 'filter')
		{
			...
			{
				$this->where('order.createtime >= ?',[strtotime($post['createtime_from'])]);
			}
			if (!empty($post['createtime_to']))
			{
				$this->where('order.createtime <= ?',[strtotime($post['createtime_to'])]);
			}
			if(!empty($post['uid']))
			{
				$this->where('order.uid=?',[$post['uid']]);
			}
			if (!empty($post['orderamount_from']))
			{
				$this->where('order.orderamount >= ?',[$post['orderamount_from']]);
			}
			if (!empty($post['orderamount_to']))
			{
				$this->where('order.orderamount <= ?',[$post['orderamount_to']]);
			}
			if (!empty($post['address_province']))
			{
				$this->where('address.province=?',[$post['address_province']]);
			}
			if (!empty($post['address_city']))
			{
				$this->where('address.city=?',[$post['address_city']]);
			}
			...
			{
				$this->where('pay_number=?',[trim($post['pay_number'])]);
			}
			if ($post['kouan'] != '')
			{
				if ($post['kouan'] == 3)
				{
					$this->where('order.need_kouan=?',[0]);
				}
				else
				{
					$this->where('order.kouan=? and order.need_kouan=?',[$post['kouan'],1]);
				}
			}
			if ($post['status'] != '')
			{
				$this->where('order.status=?',[$post['status']]);
			}
			if ($post['way_status'] != '')
			{
				$this->where('order.way_status=?',[$post['way_status']]);
			}
			
////////////////////////////////mychange///////根据地址号码来搜索	
			if ($post['address_telephone'] != '')
			{
				$this->where('address.telephone like ? or address.name like ?',['%'.trim($post['address_telephone']).'%','%'.trim($post['address_telephone']).'%']);
			}
////////////////////////////////			
			if ($post['task'] != '')
			{
	...
	function source_datatables($post,$session_uid,$session_id)
	{
		...
		$this->where('user.o_master=? or user.source=?',[$session_uid,$session_id]);
		return $this->select($parameter);
	}
	...
	function source_count($session_id,$session_uid)
	{
		...
		->where('user.o_master=? or user.source=?',[$session_uid,$session_id])
		->find('count(*)');
	...
	function source_count2($session_id,$session_uid)
	{
		...
		$result = $this
		->table('user','left join','user.id=order.uid')
		->where('user.source=? or user.oid =?',[$session_id,$session_uid])
		->find('count(*)');
	...
	function fetch(array $filter = [])
	{
		if (isset($filter['uid']))
		{
			$this->where('order.uid=?',[$filter['uid']]);
		}
		if (isset($filter['pay_status']))
		{
			$this->where('order.pay_status=?',[$filter['pay_status']]);
		}
		if (isset($filter['way_status']))
		{
			$this->where('order.way_status=?',[$filter['way_status']]);
		}
		if (isset($filter['receive']))
		{
			$this->where('order.receive=?',[$filter['receive']]);
		}
		if (isset($filter['status']))
		{
			$this->where('order.status=?',[$filter['status']]);
		}
		if (isset($filter['isdelete']))
		{
			$this->where('order.isdelete=?',[$filter['isdelete']]);
		}
