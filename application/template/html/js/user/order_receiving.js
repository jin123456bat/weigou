var receiving = datatables({
	table:$('#order_receiving table'),
	ajax:{
	...
	columnDefs:[{
		...
		render:function(data,full){
			...
			*/
			let content = `<button class="btn btn-outline btn-xs orderdetail center-block look" data-id="${data}">查看</button>`;
			/*if(full.status==1)
			{
...
//批量操作
$('#order_receiving .multipleBtn').on('click',function(){
	...
	if($(this).parents('td').find('.multipleSelect').val() == 'delete')
	{
		bootbox.confirm({
			...
			callback: function(result,a) {
				if(result) {
					receiving.addAjaxParameter('customActionType','group_action');
					receiving.addAjaxParameter('id',id);
					receiving.addAjaxParameter('customActionName','remove');
					receiving.reload();
				}
			},
		});
