...
class export extends view
{
	...
	function order()
	{
		...
		$data = $order_productModel->table('order_package', 'left join', 'order_package.id=order_product.package_id')
		...
		foreach ($data as &$value)
		{
			...
			$op = $this->model("order_product")
				->table("product", "left join", "product.id=order_product.pid")
				->where("package_id=? and product.barcode=?", [
				$value['id'],
				$value['barcode']
			])
				->find();
	...
	function product()
	{
		...
		$data = $productModel->select([
			...
			'(select name from category,category_product where category_product.cid=category.id and category_product.pid=product.id and category_product.isdelete=0 limit 1,1) as category2'
		] // 第二个分类
