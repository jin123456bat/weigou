...
class response
{
	...
	public function send()
	{
		...
		{
			if (!empty($this->_body->getContentType()))
			{
				$this->addHeader('Content-Type',$this->_body->getContentType());
			}
			if(!$this->_body->isCache())
			{
				$this->addHeader('Cache-Control','no-cache, must-revalidate, no-store');
				$this->addHeader('Pragma','no-cache');
				$this->addHeader('Expires',-1);
				$this->addHeader('Last-Modified', gmdate ('D, d M Y H:i:s',strtotime('-1 day')).' GMT');
			}
		}
