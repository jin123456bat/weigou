...
class order extends view
{
		...
		if (! empty($orderno))
		{
			...
			if (! empty($package['ship_type']) && ! empty($package['ship_number']))
			{
				...
				if ($response['resultcode'] == 200)
				{
					...
					$way = array_reverse($response['result']['list']);
					foreach ($way as &$w)
					{
						$w['datetime'] = array(
							'date' => date('Y.m.d',strtotime($w['datetime'])),
							'time' => date('H:i',strtotime($w['datetime'])),
							'week' => $week_array[date('D',strtotime($w['datetime']))],
						);
					}
					$this->assign('way', $way);
				}
	...
	function crontab()
	{
		...
		$task_user = $this->model('task_user')
			...
			->where('task_user.status=? and task_user.o_orderno is null and order.status=?', [
			0,
			1
		])
			->select([
		...
		foreach ($task_user as $main_order)
		{
			if (($_SERVER['REQUEST_TIME'] > strtotime('+1 day',strtotime($main_order['endtime']))) || ($_SERVER['REQUEST_TIME'] - $main_order['createtime'] > $main_order['day'] * 3600 * 24))
			{
				...
				$sub_order = $this->model('task_user')
					->table('`order`', 'left join', 'order.orderno=task_user.orderno')
					->where('task_user.o_orderno=? and order.status=? and task_user.status=?', [
					$main_order['orderno'],
					1,
					0
				])
		...
		switch ($this->get('type', 'alipay', 'strtolower'))
		{
			case 'alipay':
				...
				if ($this->post->notify_type == 'trade_status_sync')
				{
					...
					$pay->createParameter([
						'notify_id' => $notify_id,
						'rsa_public_key' => $this->model('system')
							->get('rsa_public_key', 'alipay'), // 支付宝公钥地址
			...
			if ($pay->auth())
			{
				...
				if (! empty($batch_no))
				{
					...
					if (empty($refund['order_product_id']))
					{
						if ($success_num >= 1)
						{
							...
							]))
							{
								...
								if (class_exists($class_name, true) && method_exists($class_name, 'refundOrder') && is_callable([
									$class_name,
									'refundOrder'
								]))
								{
					...
					else
					{
						if ($success_num >= 1)
						{
							$order_product = $this->model('order_product')
								->where('id=?', [
								$refund['order_product_id']
							])
								->find();
							...
							if ($order_product['refund'] == 2)
							{
								...
								]))
								{
									...
									]))
									{
										...
										if (class_exists($class_name, true) && method_exists($class_name, 'product') && is_callable([
											$class_name,
											'product'
										]))
										{
			...
			if (! empty($order))
			{
				...
				else
				{
					...
					else
					{
						...
						$pay->setNotifyUrl($notify_url);
						if ($this->get('return') == 'this')
						{
							$return_url = $_SERVER['HTTP_REFERER'] . '&pay=success';
						}
						else
						{
						...
						$extra = [
							'service' => $this->get('pc', 0, 'intval') ? 'create_direct_pay_by_user' : 'alipay.wap.create.direct.pay.by.user',
							'payment_type' => 1,
							'show_url' => $return_url,
