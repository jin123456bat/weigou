...
class task extends ajax
{
    function create()
    {
        ...
        if (empty($pid)) {
            $this->model("admin_log")->insertlog($admin, '团购活动新增商品失败（无商品id）');
            return new json(json::PARAMETER_ERROR);
        }

        if (!empty($this->model('task')->where('isdelete=? and pid=?', [0, $pid])->find())) {
            $this->model("admin_log")->insertlog($admin, '团购活动新增商品失败（商品已经存在）');
            return new json(json::PARAMETER_ERROR, '该商品已经存在');
        }

        if (!empty($this->model('collection')->where('pid=? and isdelete=?', [$pid, 0])->find())) {
            $this->model("admin_log")->insertlog($admin, '团购活动新增商品失败（该商品拥有多种规格，无法做为任务商品上架）');
        ...
        if ($this->model('task')->insert([
            ...
            $this->model("admin_log")->insertlog($admin, '团购活动新增商品成功，商品id：' . $pid, 1);
            return new json(json::OK, NULL, $body);
        }
        $this->model("admin_log")->insertlog($admin, '团购活动新增商品失败（参数错误）');
        return new json(json::PARAMETER_ERROR);
    }
    ...
    function remove()
    {
        ...
        if (!empty($id)) {
            if ($this->model('task')->where('id=?', [$id])->update([
                ...
                $this->model("admin_log")->insertlog($admin, '团购活动删除商品成功,商品id：'.$id, 1);
                return new json(json::OK);
            }
        }
        $this->model("admin_log")->insertlog($admin, '团购活动删除商品失败（参数错误）');
        return new json(json::PARAMETER_ERROR);
    }
    ...
    function moveup()
    {
        ...
        if (empty($id)) {
            $this->model("admin_log")->insertlog($admin, '团购活动商品排序失败（无商品号）' );
            return new json(json::PARAMETER_ERROR);
        }
        $filter = [
            'isdelete' => 0,
        ...
        $this->model("admin_log")->insertlog($admin, '团购活动商品排序成功',1);
        return new json(json::OK);
    }
    ...
    function save()
    {
        ...
        if (empty($id) || empty($price) || empty($teamnum) || empty($score)) {
            $this->model("admin_log")->insertlog($admin, '团购活动商品修改失败（参数不能为0）');
            return new json(json::PARAMETER_ERROR, '参数不能为0');
        }
        if ($teamnum < 2) {
            $this->model("admin_log")->insertlog($admin, '团购活动商品修改失败（人数不能小于2）');
        ...
        $this->model("admin_log")->insertlog($admin, '团购活动商品修改成功,商品id：'.$id,1);
        return new json(json::OK);
    }
