...
class image
{
	...
	public function water($source, $logo, $x, $y, $alpha, $savepath, $logo_w = 100, $logo_h = 100)
	{
		...
		{
			case 1:
				$dst_im = imagecreatefromgif($source);
				break;
			case 2:
			...
			case 3:
				...
				break;
			case 6:
				$dst_im = imagecreatefromwbmp($source);
				break;
			default:
				return false;
		}
		...
		switch ($src_info[2])
		{
			...
			case 3:
				...
				break;
			case 6:
				$src_im = imagecreatefromwbmp($logo);
				break;
			default:
				return false;
		}
		...
		{
			case 1:
				imagegif($dst_im, $savepath);
				break;
			case 2:
				imagejpeg($dst_im, $savepath);
				break;
			case 3:
				imagepng($dst_im, $savepath);
				break;
			case 6:
				imagewbmp($dst_im, $savepath);
				break;
			default:
				return false;
		}
	 ...
	 * @param string $string
	 *        	生成二维码的数据
	 * @param string $errorCorrentionLevel
	 *        	容错级别 默认L L|M|Q|H
	...
	public static function QRCode($string, $logo = NULL, $errorCorrectionLevel = 'L', $matrixPointSize = 4, $margin = 2)
	{
		...
		if (file_exists($path))
			include_once $path;
		$filename = ROOT . '/application/eqcode/' . md5($string) . '.png';
