...
var examine_status = datatables({
	...
	columnDefs:[{
		...
		targets:7,
		render:function(data,full){
			if(full.examine_final==0)
			{
				return '<font style="color:#FF6600;">待审核</font>';
			}
			else if(full.examine_final==2)
			{
...
$('#status table').on('click','.editBtn',function(){
	var id = $(this).data('id');
	$.post('./index.php?m=ajax&c=product&a=examine2_status',{id:id},function(response){
		if(response.code==1)
		{
			window.location = './index.php?c=html&a=product_edit&type=examine_status&id='+id;
		}
	});
	return false;
	...
	$.post('./index.php?m=ajax&c=product&a=examine_status_pass',{id:id},function(response){
		if(response.code==1)
		{
			tr.remove();
		}
	...
	$('#examineModal input[name=type]').val('examine_status');
	$('#examineModal').modal('show');
}).on('click','.lookBtn',function(){
	window.open('/index.php?c=index&a=product&id='+$(this).data('id'));
});

$('#status .search').on('submit',function(){
	examine_status.addAjaxParameter('status',$(this).find('select').val());
	examine_status.search($(this).find('input').val());
	return false;
});