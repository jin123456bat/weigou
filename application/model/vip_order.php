...
class vip_orderModel extends model
{
	...
	function vipdatatables($post, $session_id)
	{
		...
		$this->where('user.oid in (select id from user where oid=?) or user.oid=? or user.id=?', [
			$uid,
			$uid,
			$uid
		]);
		...
		if (isset($post['action']) && $post['action'] === 'filter')
		{
			...
			{
				$this->where('vip_order.orderno like ?', [
					'%' . $post['orderno'] . '%'
				]);
			}
			if (! empty($post['uid']))
			{
				$this->where('vip_order.uid=?', [
					$post['uid']
				]);
			}
			if (! empty($post['createtime_from']))
			{
				$this->where('vip_order.createtime >= ?', [
			...
			if (! empty($post['vip']))
			{
				...
				{
					case '0-1':
						$this->where('vip_order.vip_from = ? and vip_order.vip_to=?', [
							0,
							1
						]);
						break;
